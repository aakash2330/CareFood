...
function RestaurantCards({
  ...
  <div className={`flex bg-[#FCFCFC] gap-x-4 px-4 py-10 cursor-pointer lg:gap-x-10  w-[580px]    items-center shadow-xl rounded-xl my-4  transition ease-in-out delay-75 hover:scale-105 duration-200 ${searchInput == "" ? "xl:w-5/12" : "xl:w-12/12"}`}>
    ...
    <div className="flex flex-col gap-y-1 xl:w-8/12">
      <div className="text-[#515151] font-medium text-xl">{restaurant?.info?.name}</div>
      <div className="text-[#AEB0AF] tracking-wide truncate text-sm max-w-[300px]">{restaurant?.info?.cuisines.join(", ")}</div>
      ...
      <div className="flex gap-x-1 text-[#5F5F5F] text-sm pt-1 font-semibold">
        <div className="flex"> <div className="-mt-[1px] mx-1"> ⭐</div> {restaurant?.info?.avgRatingString} - </div>
